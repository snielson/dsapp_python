...
class SoapClient:
    ...
    def invoke(self, args, kwargs):
        ...
        soapenv = binding.get_message(self.method, args, kwargs, self.options)
        timer.stop()
        metrics.log.debug(
    ...
    def send(self, soapenv):
        ...
        try:
            ...
            if retxml:
                ...
                try: result.__message__ = reply.message
                except: pass
    ...
    def failed(self, binding, error):
        ...
        reply = error.fp.read()
        log.debug('http failed (error code %s (%s)):\n%s',status, reason, reply)
        if status == 500:
            if len(reply) > 0:
...
class SimClient(SoapClient):
    ...
    # XXX DAP: start kwood copy/paste with more efficient attachment handling with using __inject
    def send(self, msg):
        """
        Send soap message.
        ...
        try:
            ...
            if e.httpcode in (202,204):
                ...
                result = self.failed(binding, e)
        return result
    # XXX DAP: end kwood copy/paste with more efficient attachment handling with using __inject
    ...
    def invoke(self, args, kwargs):
        ...
        # XXX DAP: this was commented by kwood to remove (probably) unnecessary marshalling/unmarshalling step of large unicode attachment strings going through SAX and converting back to string.
        #sax = Parser()
    ...
    def __reply(self, reply, args, kwargs):
        """ simulate the reply """
        binding = self.method.binding.input
        msg = binding.get_message(self.method, args, kwargs, self.options)
