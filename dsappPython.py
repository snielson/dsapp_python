	...
	if 'No updates found' in zLU[0]:
		...
		if ds.askYesOrNo('List %s packages' % repo):
			...
			if ds.askYesOrNo("Force install %s packages" % repo):
				...
				spinner.stop(); print
				time2 = time.time()
				ds.logger.info("Force update Mobility package took %0.3f ms" % ((time2 - time1) * 1000))
				print "\nPlease run 'sh %s/update.sh' to complete the upgrade" % dirOptMobility
	else:
		print "Updating Mobility.. ",
		ds.logger.info('Updating Mobility..')
		spinner.start(); time.sleep(.000001)
		time1 = time.time()
		...
		spinner.stop(); print
		time2 = time.time()
		ds.logger.info("Updating Mobility package took %0.3f ms" % ((time2 - time1) * 1000))
		...
		with open(dsappSettings, 'wb') as cfgfile:
			Config.write(cfgfile)
		setVariables()
		ds.logger.info('Updating Mobility schema..')
		time1 = time.time()
		...
		time2 = time.time()
		ds.logger.info("Updating Mobility schema took %0.3f ms" % ((time2 - time1) * 1000))
